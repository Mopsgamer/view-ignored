...
function showHelp() {
	...
	console.log(`\n${header}`)
	console.log(
		styleText("dim", "  Pre-v1 internal utility for bug hunting and discrepancy identification.\n"),
	)

	console.log(styleText("bold", "Usage:"))
	console.log(
		`  vign-diff ${styleText("blue", "[command]")} ${styleText("blue", "[target]")} [flags]\n`,
	)
	...
	console.log(
		`  -i, --issue     ${styleText("dim", "Open prefilled GitHub issue on discrepancy.")}`,
	)
...
async function runTarget(
	name: string,
	options: { issue: boolean; list: boolean; verbose: boolean },
): Promise<boolean> {
	const info = SUPPORTED_TARGETS[name]
	if (!info) {
		if (name !== "all" && name)
			process.stdout.write(styleText("yellow", `! Warning: Target "${name}" is not supported.\n`))
		return false
	}
	...
	if (discrepancies.length > 0) {
		...
		const reports = discrepancies.map((d) => ({
			issue: d.issue,
			origin:
				d.match.kind === RuleMatchKind.external
					? d.match.source.path
					: d.match.kind === RuleMatchKind.internal
						? "internal"
						: "none",
			path: d.file,
			pattern:
				d.match.kind === RuleMatchKind.external || d.match.kind === RuleMatchKind.internal
					? d.match.pattern
					: undefined,
			ruleMatch: d.match,
		}))
		...
		if (options.issue) {
			...
			const body = encodeURIComponent(
				`Discrepancy report for target **${name}**:\n\n\`\`\`json\n${JSON.stringify(reports, null, 2)}\n\`\`\``,
			)
...
async function main() {
	...
	let hasDiff = false
	if (targetArg === "all") {
		for (const name of Object.keys(SUPPORTED_TARGETS)) {
			// eslint-disable-next-line no-await-in-loop
			if (await runTarget(name, options)) hasDiff = true
		}
	} else {
