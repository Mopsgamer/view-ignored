...
export function resH(p: string) {
	if (p.charCodeAt(0) === 126 && p.charCodeAt(1) === 47) return join(HOME, p.slice(2))
	return p
}

export function resP(base: string, p: string) {
	p = resH(p)
...
export function getCache<K, V>(wm: WeakMap<FsAdapter, Map<K, V>>, fs: FsAdapter) {
	let m = wm.get(fs)
	...
	fs.readFile(path, (err, res) => {
		...
		for (let i = 0; i < len; i++) {
			loadRec(fs, resP(dir, inc[i]!), gitDir, branch, sig, (v) => {
				vals[i] = v
				if (--pending === 0) {
					for (let j = 0; j < len; j++) if (vals[j]) merge(p, vals[j])
					cb(p)
				}
			})
		}
