...
export function resolvePath(base: string, p: string): string {
	...
	// Absolute path or Windows drive
...
function parseKeyValuePair(text: string, i: number, len: number) {
	...
	if (i >= len || text.charCodeAt(i) === 10) {
		return { key: null, nextIdx: i + 1, val: null }
	}
...
function testPattern(pat: string, str: string, mode: MatchMode = MatchMode.normal) {
	...
	if (!c) {
		c = patternCompile(pat, undefined, mode)
...
/**
 * Gets include paths from parsed config.
 */
...
/**
 * Recursively loads git configurations.
 */
export function loadRec(
	...
	if (cached && !gitDir) return cb(cached)

	fs.readFile(path, (err, res) => {
		if (err) return cb(null)

		const parsed = parseGit(res!.toString())
		if (!gitDir) cache.set(path, parsed)
		...
		for (let i = 0; i < len; i++) {
			loadRec(fs, resolvePath(dir, includes[i]!), gitDir, branch, sig, (v) => {
				...
				if (--pending === 0) {
					for (let j = 0; j < len; j++) {
						if (results[j]) mergeConfig(parsed, results[j])
					}
					cb(parsed)
				}
			})
