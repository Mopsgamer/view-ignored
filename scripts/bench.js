...
const benchmarkFiles = positionals.length > 0
	? positionals
...
function compareBenchmarks(current, base) {
	...
	for (const [name, curr] of currentMap) {
		...
		const baseBench = baseBenchWrapper ? (baseBenchWrapper.runs ? baseBenchWrapper.runs[0] : baseBenchWrapper) : null;
		...
		if (baseBench && baseBench.stats && baseBench.stats.samples) {
			...
			const sp = Math.sqrt(((n1 - 1) * currStats.stdDev ** 2 + (n2 - 1) * baseStats.stdDev ** 2) / Math.max(1, df));
			const half = (z * sp * normer) * 100 / (baseStats.mean || 1);

			const diffPercent = (currStats.mean - baseStats.mean) * 100 / (baseStats.mean || 1);

			const isSig = (diffPercent >= 1 && (diffPercent - half) >= 1) || (diffPercent <= -1 && (diffPercent + half) <= -1);
		...
		rows.push({
			...
			outliers: `${currStats.outliers.toString().padStart(4)} (${Math.round(currStats.outliers * 100 / (currStats.sampleCount || 1)).toString().padStart(2)}%)`,
...
function printTable(rows) {
	...
	const headers = ["Benchmark", "Measurement (mean \u00b1 std.dev)", "Min \u2026 Max", "Outliers", "Ratio"];
	const colWidths = [headers[0].length, headers[1].length, headers[2].length, headers[3].length, headers[4].length];
